...
dat <- download.NDVI("https://raw.githubusercontent.com/jslingsby/BIO3019S_Ecoforecasting/master/data/modisdata.csv")
...
>*Answer 1: It doesn't increase linearly with time because as time passes, the environmental conditions fluctuate and the differnt fynbos species are responding differently to those fluctations.
...
>*Answer 2: For both alpha and lambda their effects (parameter estimates) decreased when we added more paramters to the model. The effect of gamma increased. This could be because of some form of collinearity between the gamma parameter and another parameter such that when those other parameters are added, gamma has a larger effect.
...
>*Answer 3: According to the Residual sum of squares presented by anova, model 2(the full model) is better. 

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4: There are 135 degrees of freedom for model 1 since we have 138 data points and we estimated 3 parameters (138-5). Model 2 has 133 degrees of freedom because 5 parameters are being estimated in that model (138-5).
...
>*Answer 5: The full model is the preferred model. It has a much lower AIC than that of the simple model and the difference between the models' AICs is way more than 3 indicating a significant difference. The full model is doing a better job at explaining the pattern in the data.
...
>*Answer 6: AIC requires the fitted object ie. the model to have a loglik method for extracting its log-likelihood. If it doesn't, the AIC for that model can be obtained but cannot be compared to that of models whose log-likelihood can be extracted. NLS does not have this method and MLE does thus their AICs cannot be compared.
